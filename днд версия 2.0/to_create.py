...
def main():
    ...
    def generator_status():
        ...
        while True:
            ...
            txt = "Введите 'прекратить генерацию' чтобы завершить генерацию "
            txt += "или 'продолжить генерацию' чтобы продожить генерацию чисел"
            answer = str(input(txt).lower())
            match answer:
                case "прекратить генерацию":
                    character["CHstts"] = stats
                    break
    ...
    try:
        races_info_path = os.path.join(way_to_dir, "races.json")
    ...
    try:
        bgs_info_path = os.path.join(way_to_dir, "bgrounds.json")
